            ...
            <div className="flex gap-5 max-md:flex-col max-md:gap-0">
                ...
                <form onSubmit={checkOut} className="flex flex-col h-[520px] ml-5 w-[38%] max-md:ml-0 max-md:w-full">
                    <div className="flex flex-col grow px-5 py-5 mx-auto w-full font-medium bg-indigo-700 rounded-3xl max-md:mt-10">
                        ...
                        <input id="nameOnCard" disabled={!items.length} className="px-5 py-4 mt-3 text-xs bg-indigo-500 rounded-md text-stone-300 max-md:pr-5" type="text" placeholder="Name" aria-label="Name on card" required />
                        <label className="mt-5 text-sm text-white" htmlFor="cardNumber">Card Number</label>
                        <input id="cardNumber" disabled={!items.length} className="px-5 py-4 mt-2.5 text-xs bg-indigo-500 rounded-md text-stone-300 max-md:pr-5" type="number" placeholder="1111 2222 3333 4444" required />
                        <div className="flex gap-2 mt-5">
                            <div className="flex flex-col flex-1">
                                <label className="text-sm text-white" htmlFor="expirationDate">Expiration date</label>
                                <input id="expirationDate" disabled={!items.length} className="px-5 py-3.5 mt-1.5 text-xs bg-indigo-500 rounded-md text-stone-300 max-md:pr-5" type="number" placeholder="mm/yy" aria-label="Expiration date" required />
                            </div>
                            <div className="flex flex-col flex-1">
                        ...
                        <button
                            ...
                            className={`flex gap-5 justify-between px-6 py-5 mt-7 w-full text-base text-white rounded-xl max-md:pl-5
              ${!items.length ? 'cursor-not-allowed bg-gray-300' : 'bg-teal-300'}`}
                        >
                            ...
                            <p className="flex items-center gap-2">
                                ...
                                <img
                                    loading="lazy"
                                    src="https://cdn.builder.io/api/v1/image/assets/TEMP/af6b0acbfc3f32c227a0e7775da13090d3bc97c87e29eac3d4765ae8abfbe4a1?apiKey=7e724f8dbe7341e8950e7325e9acf7be&"
                                    alt="Checkout"
